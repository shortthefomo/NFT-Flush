            ...
            <table class="table">
                ...
                <tbody>
                    <tr v-for="row in NFTokenOffers" @click="selectedRow(row)" :class="highlights(row)">
                        <td v-if="typeof row['Amount'] === 'object'">{{row['Flags'] == 1 ? 'sell':'buy'}} {{numeralFormat((row['Amount'].value), '0,0') }} {{currencyHexToUTF8(row['Amount'].currency)}}</td>
                        <td v-else>{{row['Flags'] == 1 ? 'sell':'buy'}} {{numeralFormat((row['Amount']/1_000_000), '0,0[.]00000000')}} XRP</td>
